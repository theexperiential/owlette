...
/**
 * Http-shape coverage for POST /api/cortex/cancel-tool
 * (cortex-async-turns wave 2.4).
 *
 * Authz matrix:
 *   - unauthenticated → 401
...
// Full replacement — never load the real cortex-utils.server (owned by a
...
import { POST } from '@/app/api/cortex/cancel-tool/route';
...
function cancelRequest(body: Record<string, unknown> = {}): NextRequest {
  return new NextRequest('http://localhost/api/cortex/cancel-tool', {
    method: 'POST',
...
describe('POST /api/cortex/cancel-tool — authz', () => {
...
describe('POST /api/cortex/cancel-tool — dispatch', () => {
...
describe('POST /api/cortex/cancel-tool — rate limiting', () => {
