...
/**
 * Tool-wiring coverage for POST /api/cortex/autonomous.
 *
...
const mockIsCortexEnabled = jest.fn();
...
jest.mock('@/lib/cortex-utils.server', () => ({
  ...
  SERVER_SIDE_TOOLS: jest.requireActual('@/lib/cortex-utils.server').SERVER_SIDE_TOOLS,
  ...
  isCortexEnabled: (...args: unknown[]) => mockIsCortexEnabled(...args),
}));

const mockDispatchToolCall = jest.fn();
const mockDispatchExistingCommand = jest.fn();

jest.mock('@/lib/cortex/dispatch.server', () => ({
...
jest.mock('@/lib/cortex-escalation.server', () => ({ escalate: jest.fn() }));
...
const cortexEventsQuery: FakeQuery = {
  where: () => cortexEventsQuery,
  limit: () => cortexEventsQuery,
...
import { POST } from '@/app/api/cortex/autonomous/route';
import { SERVER_SIDE_TOOLS } from '@/lib/cortex-utils.server';
...
function request(): NextRequest {
  return new NextRequest('http://localhost/api/cortex/autonomous', {
    method: 'POST',
...
beforeEach(() => {
  ...
  mockIsCortexEnabled.mockResolvedValue(true);
