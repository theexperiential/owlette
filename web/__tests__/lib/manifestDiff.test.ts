 ...
 *
 * tests for web/lib/manifestDiff.ts (roost wave 3.7).
 */
...
function entry(path: string, hashes: string[]): ManifestFileEntry {
  return {
    path,
...
describe('diffManifests', () => {
  ...
  it('identical manifests → everything unchanged', () => {
    const files = [entry('a.toe', ['h1']), entry('b.toe', ['h2'])];
    const d = diffManifests(files, files);
  ...
  it('same chunks in DIFFERENT ORDER are flagged changed (regression)', () => {
    // the manifest preserves chunk order — reordered chunks = different bytes.
    const from = [entry('a.toe', ['h1', 'h2'])];
...
describe('summariseDiff', () => {
  it('reports zero changes when manifests are identical', () => {
