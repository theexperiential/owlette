...
/**
 * Unit tests for `web/lib/cortex/turnRunner.server.ts`
 * (cortex-async-turns wave 2.2).
 *
 ...
 *      persist to `chats/{chatId}` with the useCortex-mirrored shape;
...
jest.mock('@/lib/cortex-utils.server', () => ({
  ...
  getCortexRequireTier3Approval: jest.fn(),
  getOnlineMachines: jest.fn(),
  resolveCortexMaxTier: jest.fn(),
...
jest.mock('@/lib/cortex/categorizeChat.server', () => ({
  __esModule: true,
  categorizeNewChat: jest.fn(),
}));

import * as turnStore from '@/lib/cortex/turnStore.server';
import * as cortexUtils from '@/lib/cortex-utils.server';
...
} from '@/lib/cortex/turnRunner.server';
...
beforeEach(() => {
  ...
  (cortexUtils.resolveLlmConfig as jest.Mock).mockResolvedValue({
    provider: 'anthropic',
    apiKey: 'k',
  });
  (cortexUtils.getCortexRequireTier3Approval as jest.Mock).mockResolvedValue(true);
  (cortexUtils.getOnlineMachines as jest.Mock).mockResolvedValue(['machine-1', 'machine-2']);
  (cortexUtils.resolveCortexMaxTier as jest.Mock).mockReturnValue(3);
  (cortexUtils.buildExecutableTools as jest.Mock).mockImplementation(stubBuildTools);
...
describe('startTurn — happy path', () => {
  it('streams text, writes snapshots, persists the final chat, and finishes the turn', async () => {
    ...
    // Final chat persist mirrors the useCortex client shape (new conversation).
  ...
  it('site-wide mode: site prompt, online-machine fan-out, and site persist shape', async () => {
    ...
    expect(cortexUtils.buildExecutableTools).toHaveBeenCalledWith(
      fakeDb,
      SITE_ID,
      '__site__',
      CHAT_ID,
...
describe('startTurn — site-wide dangling-tool recovery aggregation', () => {
  ...
  beforeEach(() => {
    capturedPrompt = undefined;
    (cortexUtils.getOnlineMachines as jest.Mock).mockResolvedValue([
      'machine-1',
      'machine-2',
      'machine-3',
    ]);
...
describe('startTurn — heartbeat abort decision', () => {
  ...
  function blockingToolSetup(observe: { aborted: boolean }) {
    (cortexUtils.buildExecutableTools as jest.Mock).mockImplementation(() => ({
      get_metrics: tool({
...
describe('startTurn — error paths', () => {
  ...
  it('setup failure (llm config): stream still terminates with an error chunk', async () => {
    (cortexUtils.resolveLlmConfig as jest.Mock).mockRejectedValue(
