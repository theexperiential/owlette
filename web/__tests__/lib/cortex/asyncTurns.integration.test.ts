...
/**
 * Integration tests for the cortex async-turn architecture
 * (cortex-async-turns wave 5.1).
 *
 ...
 * `cortex-utils.server` (`buildExecutableTools` / `executeToolOnAgent`, so the
...
describe('tier-3 approval round-trip through the runner', () => {
  ...
  async function runTurn1() {
    ...
    // getCortexRequireTier3Approval resolves true (default-on gate).
    currentModel = new MockLanguageModelV3({
      doStream: async () => ({
        stream: simulateReadableStream({
          chunks: [
