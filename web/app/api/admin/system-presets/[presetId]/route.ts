/**
 * GET    /api/admin/system-presets/{presetId}
 *        → fetch a single system preset doc.
 *
 * PATCH  /api/admin/system-presets/{presetId}
 *        → partial update (only provided fields are written).
 *          Requires SYSTEM_PRESET_MANAGE.
 *
 * DELETE /api/admin/system-presets/{presetId}
 *        → hard-delete the preset. Idempotent on non-existent ids.
 *          Requires SYSTEM_PRESET_MANAGE.
 *
...
export const GET = authorizedPlatformHandler<RouteParams>({
  ...
  try {
    ...
    return problemFromError(err, 'admin/system-presets/[presetId]:GET');
  }
});
...
export const PATCH = authorizedPlatformHandler<RouteParams>({
  ...
  try {
    ...
    return problemFromError(err, 'admin/system-presets/[presetId]:PATCH');
  }
});
...
export const DELETE = authorizedPlatformHandler<RouteParams>({
  ...
  try {
    ...
    return problemFromError(err, 'admin/system-presets/[presetId]:DELETE');
  }
});
