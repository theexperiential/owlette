/**
 * GET  /api/platform/system-presets
 * POST /api/platform/system-presets
 *
 * Platform-level system presets are global software-deployment templates
...
export const GET = authorizedPlatformHandler({
  ...
  try {
    ...
    return problemFromError(err, 'platform/system-presets:GET');
  }
});
...
export const POST = authorizedPlatformHandler({
  ...
  try {
    ...
    return problemFromError(err, 'platform/system-presets:POST');
  }
});
