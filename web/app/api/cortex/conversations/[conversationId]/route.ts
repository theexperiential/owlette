 ...
 *
 * This is the sole implementation; the legacy `/api/chat/{conversationId}`
 * compatibility alias was removed (no public consumers existed).
 *
 * All three verbs require `chat=<siteId>:write`. siteId is read from the
 * conversation document (the URL only carries the conversation id) and passed
 * into `requireChatAuthAndScope` so api-key callers must hold the correct
 * site-scoped chat permission.
 */
...
export async function POST(request: NextRequest, { params }: RouteContext) {
  try {
        ...
        const streamResult = await runCortexStream({
          ...
          onAssistantText: async (assistantText) => {
            ...
            try {
              ...
              console.warn('[cortex/conversations] failed to persist assistant turn:', err);
            }
          },
        });
    ...
    return problemFromError(err, 'cortex/conversations/[conversationId]:POST');
  }
}
...
export async function PATCH(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    return problemFromError(err, 'cortex/conversations/[conversationId]:PATCH');
  }
}
...
export async function DELETE(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    return problemFromError(err, 'cortex/conversations/[conversationId]:DELETE');
  }
}
