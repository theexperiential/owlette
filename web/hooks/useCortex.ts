 ...
 * - Async-turn recovery (cortex-async-turns): live subscription to the
 ...
 *   runner in web/lib/cortex/turnRunner.server.ts is the persist authority).
 */
...
// Mirrors TURN_STALE_MS in web/lib/cortex/turnStore.server.ts: a `running`
...
export function useOwletteChat({ siteId, machineId, machineName, onChatPersisted }: UseChatOptions) {
  ...
  // idle). Read by stop() to POST /api/cortex/stop for a real server cancel.
  ...
  const chat = useAIChat({
    ...
    onFinish: () => {
      ...
      // conversations (web/lib/cortex/turnRunner.server.ts) — persisting here
  ...
  const cancelTool = useCallback(async (toolCallId: string) => {
    ...
    await Promise.allSettled(
      Object.entries(perMachine).map(async ([machineId, { commandId }]) => {
        try {
          const response = await fetch('/api/cortex/cancel-tool', {
            method: 'POST',
  ...
  // the stream doc reports a running turn, POST /api/cortex/stop to flip it to
  ...
  const stop = useCallback(async () => {
    ...
    if (running && turnId) {
      try {
        const response = await fetch('/api/cortex/stop', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  return {
    ...
    // Real server-side cancel (POSTs /api/cortex/stop, then aborts the local
