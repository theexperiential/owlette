...
/**
 * Subscribe to a site's tier-3 Hoot approval policy
 * (`sites/{siteId}/settings/cortex.requireTier3Approval`).
 *
 * Defaults to `true` (gate on) when the doc/field is absent or unreadable,
 * mirroring the server-side `getHootRequireTier3Approval` default — so the
...
export function useHootApprovalSetting(siteId: string) {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (error) => {
        console.error('Failed to read hoot approval setting:', error);
