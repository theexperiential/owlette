 ...
 * machine status doc — NOT the config doc). When `false`, hoot tool
...
export interface SetHootEnabledInput {
  machineId: string;
  enabled: boolean;
}

export interface SetHootEnabledResult {
  machineId: string;
  enabled: boolean;
}

export async function setHootEnabled(
  ctx: ActionContext,
  input: SetHootEnabledInput,
): Promise<SetHootEnabledResult> {
  ...
  logger.info(`Hoot ${input.enabled ? 'enabled' : 'disabled'} on ${input.machineId}`, {
    context: 'actions/setHootEnabled',
  });
