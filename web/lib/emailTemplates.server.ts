...
/* ------------------------------------------------------------------ */
/*  Subscription health emails (billing-system wave 4.3)               */
/* ------------------------------------------------------------------ */

/**
 * The three subscription-health events that page somebody. Declared here
 * rather than in `@/lib/billing/billingHealthAlerts.server` for the same
 * reason as {@link TrialEmailMilestone}: the sender imports the builders
 * from this module, so owning the union here keeps the dependency edge
 * one-directional (the alert module re-exports it as
 * `BillingHealthAlertKind`).
 */
export type BillingHealthEmailKind =
  | 'subscription_canceled'
  | 'payment_failed'
  | 'past_due';

/** Short human label for a kind, used in ops subjects and table rows. */
export const BILLING_HEALTH_LABEL: Record<BillingHealthEmailKind, string> = {
...
/**
 * Customer-facing copy per kind.
 *
 * Deliberately calm: two of the three are recoverable with a card update,
 * and the third deletes nothing. Every body says what is *still working*
 * before it says what is paused, because the first question a customer has
 * on reading one of these is "did I just lose my fleet?".
 */
const BILLING_HEALTH_OWNER_COPY: Record<
  BillingHealthEmailKind,
  { subject: string; heading: string; accent: string; lead: string; body: string; cta: string }
> = {
  subscription_canceled: {
    subject: 'your owlette subscription was canceled',
    heading: 'your subscription was canceled',
    accent: EMAIL_COLORS.red,
    lead: 'your owlette subscription has been canceled.',
    body: 'your machines keep reporting in and nothing has been deleted — metrics, logs, and deployments are all still there. process control, deploys, restarts, and display config are paused until you choose a plan.',
    cta: 'choose a plan to reactivate',
  },
  payment_failed: {
    subject: "we couldn't process your owlette payment",
    heading: "we couldn't process your payment",
    accent: EMAIL_COLORS.amber,
    lead: "the latest charge for your owlette subscription didn't go through.",
    body: 'this is usually an expired or declined card. we retry automatically over the next few days, and nothing is paused in the meantime — updating your payment method under account settings → billing → manage billing fixes it straight away.',
    cta: 'open your dashboard',
  },
  past_due: {
    subject: 'your owlette account is past due',
    heading: 'your account is past due',
    accent: EMAIL_COLORS.amber,
    lead: 'a payment on your owlette subscription is overdue.',
    body: 'everything keeps working while we retry the charge. if the retries run out the subscription is canceled and control actions pause, so updating your payment method under account settings → billing → manage billing is the safest fix. nothing is deleted either way.',
    cta: 'open your dashboard',
  },
};

/**
 * Build the account owner's subscription-health email.
 *
 * Transactional, exactly like {@link buildTrialEmail}: no `unsubscribeUrl`
 * key is passed at all, so `wrapEmailLayout` renders no "manage alerts ·
 * unsubscribe" footer. There is no opting out of being told your card was
 * declined — the alternative is a silent cancellation.
 *
 * @param kind      which health event this is
 * @param actionUrl dashboard deep-link the CTA points at
 */
export function buildBillingHealthOwnerEmail(
  kind: BillingHealthEmailKind,
  actionUrl: string,
): { subject: string; html: string } {
  const c = BILLING_HEALTH_OWNER_COPY[kind];

  const ctaButton =
    `<table cellpadding="0" cellspacing="0" role="presentation" style="margin:24px 0;">` +
    `<tr><td style="border-radius:6px;background:${EMAIL_COLORS.cyan};">` +
    `<a href="${actionUrl}" style="display:inline-block;padding:12px 28px;color:${EMAIL_COLORS.bodyBg};text-decoration:none;font-weight:700;font-size:14px;border-radius:6px;">${c.cta}</a>` +
    `</td></tr></table>`;
  ...
  const content = `
    ...
    <p style="margin:0 0 8px;color:${EMAIL_COLORS.muted};">${c.body}</p>
    ${ctaButton}
    <p style="margin:0;color:${EMAIL_COLORS.muted};font-size:12px;">if the button doesn't work, copy and paste this link into your browser:<br><span style="color:${EMAIL_COLORS.cyan};word-break:break-all;">${actionUrl}</span></p>
  `;
...
/** Ops-facing facts about the account that tripped the alert. */
export interface BillingHealthOpsDetails {
  /** Owlette account uid — the `customers/{uid}` key. */
  uid: string;
  /** Stripe event id, so the alert maps back to a webhook delivery. */
  eventId: string;
  /** Stripe event type that produced it. */
  eventType: string;
  /** Resolved `billingState` after the event was applied. */
  billingState: string;
  /** Mirrored `subscriptionStatus` after the event was applied. */
...
/**
 * Build the superadmin's subscription-health email.
 *
 * Ops-facing, so it leads with identifiers rather than reassurance — the
 * reader's next action is opening the Stripe dashboard or the account. The
 * customer's email address is deliberately *not* included: the uid resolves
 * to the account in one lookup, and an ops alert is not a reason to widen
 * where PII lands. Every value goes through `emailDataTable`, which escapes.
 */
export function buildBillingHealthOpsEmail(
  kind: BillingHealthEmailKind,
  details: BillingHealthOpsDetails,
): { subject: string; html: string } {
  const label = BILLING_HEALTH_LABEL[kind];
  const accent = kind === 'subscription_canceled' ? EMAIL_COLORS.red : EMAIL_COLORS.amber;

  const content = `
    <h2 style="color:${accent};margin:0 0 12px;font-size:18px;font-weight:700;text-transform:lowercase;">${label}</h2>
    <p style="margin:0 0 20px;color:${EMAIL_COLORS.muted};">a subscription health event fired for an owlette account.</p>
    ${emailDataTable([
      { label: 'event', value: label, highlight: accent },
      { label: 'account uid', value: details.uid },
      { label: 'billing state', value: details.billingState },
      { label: 'subscription status', value: details.subscriptionStatus },
      { label: 'stripe object', value: details.objectId },
      { label: 'stripe event', value: `${details.eventType} (${details.eventId})` },
      { label: 'processed at', value: emailTimestamp(details.occurredAt) },
      { label: 'environment', value: ENV_LABEL },
    ])}
  `;

  return {
    subject: safeEmailSubject(`[${ENV_LABEL}] billing: ${label} — ${details.uid}`),
    html: wrapEmailLayout(content, { preheader: `${label} — ${details.uid}` }),
  };
}
