...
Required only if you want autonomous Cortex (AI auto-investigates process crashes). Also requires the site owner to have saved an LLM API key in their account settings — autonomous runs spend the owner's key — and autonomous mode enabled (`sites/{siteId}/settings/cortex` → `autonomousEnabled: true`).
