...
description: "Complete reference for all environment variables used by the owlette web dashboard."
---
...
## cron (required for the scheduled endpoints)

| variable | description |
|----------|-------------|
| `CRON_SECRET` | Shared secret authenticating every scheduled endpoint — sent as `X-Cron-Secret` on the six `/api/cron/*` routes and as `Authorization: Bearer` on `/api/hoot/escalation`. Each environment has its own value. See [scheduled endpoints](/docs/setup/web-deployment#scheduled-endpoints). |
...
| `INSTATUS_COMPONENT_CORTEX_CHAT_ID` | Component id for hoot chat |
...
## autonomous hoot (optional)

| variable | description |
|----------|-------------|
| `CORTEX_INTERNAL_SECRET` | Shared secret for internal auth between alert route and autonomous hoot endpoint |
...
Required only if you want autonomous hoot (AI auto-investigates process crashes). Also requires the site owner to have saved an LLM API key in their account settings — autonomous runs spend the owner's key — and autonomous mode enabled (`sites/{siteId}/settings/cortex` → `autonomousEnabled: true`).
