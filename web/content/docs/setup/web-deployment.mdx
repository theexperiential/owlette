...
| `/api/cron/billing-trial-lifecycle` | daily | `X-Cron-Secret` | trial expiries, reminder emails, post-expiry alert cutoff |
| `/api/cron/billing-usage-report` | daily 03:00 UTC | `X-Cron-Secret` | reports machine counts + roost storage overage to Stripe meters |
...
policy rather than merely degrading a feature. The billing pair is similar:
unscheduled, trials never expire and reminder emails never send, and Stripe
meters receive no usage — invoices silently bill zero.

`/api/cortex/escalation` is the odd one out: it takes
...
### cortex escalation backstop

Only needed when autonomous Cortex is enabled for a site. It is a backstop poller,
so a missed run delays escalations rather than losing them. Every 5 minutes:

```text
GET https://<your-app>/api/cortex/escalation
Authorization: Bearer <CRON_SECRET value>
```

Note the header: this route reads `Authorization: Bearer`, unlike the five
...
- [ ] Cron calls configured for **every route in the scheduled-endpoints table above**, in **each** environment — health-check (every 5 min), process-alerts + display-alerts (every 3 min), status-ping (every 60 s), retention (daily), billing-trial-lifecycle (daily), billing-usage-report (daily 03:00 UTC)
