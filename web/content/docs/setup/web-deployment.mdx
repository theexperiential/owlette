...
| `/api/cron/talons` | every 60s | `X-Cron-Secret` | claims due talons (schedule triggers), missed-fire grace, stale-run recovery |
...
Changing the window means changing **both** `METRICS_RETENTION_DAYS` /
`LOGS_RETENTION_DAYS` in the route and the stated periods in the privacy policy.
They are a pair; changing one alone makes the policy inaccurate.
