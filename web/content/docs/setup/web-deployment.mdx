...
- [ ] Cron calls configured for **every route in the scheduled-endpoints table above**, in **each** environment — health-check (every 5 min), process-alerts + display-alerts (every 3 min), status-ping (every 60 s), retention (daily)
