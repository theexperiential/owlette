...
> note: `whoami` predates the `{ ok, data }` wrapper introduced for new commands. its envelope is the raw shape above for backward compatibility — see [overview](/docs/cli/overview#json-envelope-schema) for the full envelope policy.
