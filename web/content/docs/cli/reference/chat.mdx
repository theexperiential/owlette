---
title: "chat"
description: "owlette chat drives hoot ai conversations from the terminal. site-scoped (every verb requires --site), with optional machine narrowing on new. tier: ready — all five verbs currently call the /api/cortex/conversations/* compatibility aliases; /api/hoot/conversations/* is the canonical family and /api/chat/* is an older alias set."
---
...
| `--title <title>` | no | human-readable title (hoot auto-titles otherwise) |
...
- `0` — success
- `1` — generic error (network, 5xx, malformed stream, upstream hoot error, bad `--limit`, missing required flag, non-tty without `--yes`)
...
- **scope**: site-scoped on every verb; `new` accepts an optional `--machine` for single-machine hoot sessions.
...
- **see also**: hoot API docs at [docs/api/hoot.md](/docs/api/hoot); per-machine context is fetched server-side from the machine's last heartbeat and process snapshot.
