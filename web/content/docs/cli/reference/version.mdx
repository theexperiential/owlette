...
> note: `version` predates the `{ ok, data }` wrapper. its json envelope is the raw shape above for compatibility with scripts written against earlier cli versions — see [overview](/docs/cli/overview#json-envelope-schema).
