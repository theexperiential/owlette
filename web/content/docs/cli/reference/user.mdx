---
title: "user"
description: "owlette user manages platform users — promoting, demoting, granting site access, and soft-deleting accounts. superadmin token required on every verb. tier: ready — all seven verbs are wired to /api/users/* and covered by the public API contract."
---
...
- `1` — generic error (network, 5xx, conflict not specifically handled, bad `--role`, missing `--sites`, non-tty without `--yes`)
