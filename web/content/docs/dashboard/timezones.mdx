---
title: "timezones"
description: "Owlette tracks three independent timezone actors and lets each user choose which one acts as the reference frame when the dashboard renders absolute timestamps."
---
...
|---------|---------------|-------|
| **process launch windows** | Machine local timezone | The agent evaluates scheduled process windows against each machine's own Windows clock, so "09:00 Mon-Fri" means 09:00 on that machine. The dashboard editor's outside-window hint uses the site's timezone as its best prediction of the machines' clocks, but the machine's clock is what decides. (Evaluating windows in the site's configured timezone fleet-wide is planned but not yet wired.) |
