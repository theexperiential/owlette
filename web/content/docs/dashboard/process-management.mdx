...
3. Fill in the current dialog fields:
    ...
    - **schedule configuration**: always open in `scheduled` mode, and available in `off` or `always` from the **configure schedule** button at the end of the launch-mode control. Edit one or more blocks with days and start/stop ranges in the site's timezone. Blocks saved from `off` or `always` are stored with the process and do not change its launch mode.
