...
a hoot output aimed at one machine is **skipped**, not failed, when that machine is offline (`machine_offline`) or hoot is switched off on it from the hoot header (`hoot_disabled`): the reason is recorded on the run, nothing is dispatched, and the talon keeps its clean record. a site-wide directive has no single machine to check and runs against whichever machines are online at the time.
