...
Cortex chat access follows site read access. Cortex actions that execute commands or write machine configuration require `MACHINE_EXEC_COMMAND` or `MACHINE_CONFIG_WRITE`. LLM keys are per-user and are managed by each person in their own account settings — there is no admin-managed shared key.
