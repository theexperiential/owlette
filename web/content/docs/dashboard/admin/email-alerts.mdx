...
| **hoot escalation** | Autonomous hoot cannot resolve an issue and marks an escalation pending | `cortexAlerts` | `POST /api/cortex/escalation` or cron-style `GET /api/cortex/escalation` sends escalation emails |
...
| `GET /api/cortex/escalation` | cron-style polling | `Authorization: Bearer <CRON_SECRET>` | processes pending hoot escalation events |
