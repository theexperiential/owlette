---
title: "configuration"
description: "The agent can be configured locally via the GUI, remotely from the web dashboard, or by editing config.json directly. The local source file is C:\\ProgramData\\Owlette\\config\\config.json."
---
...
![the owlette configuration gui — process list and per-process details](/docs-screens/agent.png)
...
| **Footer** | `join site` button, overflow `...` menu (config, logs, docs, feedback/bug), and version info |
...
flowchart TD
    ...
    FS --> GUI["local GUI<br/>config.json"]
