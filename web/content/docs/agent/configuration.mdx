...
| area | contents |
|---------|----------|
| **Processes list** | Every monitored process, in launch order, with a status dot from the service's live table. Drag a row to reorder; right-click one for restart, kill, duplicate, and delete. Drop an app, a script, a TouchDesigner project, or a Unity build folder anywhere in the window to add it. |
| **Process settings** | Launch mode and schedule, name, executable path, file path or arguments, working directory, startup delay, priority, initialization wait, visibility, and relaunch attempts. There is no save button — a field writes itself when it loses focus or takes an enter key. |
| **Menu** (top right) | Join or leave a site, open `config.json`, open the logs folder, open these docs, and submit a bug report. |
| **Footer** | One sentence saying whether this machine is being supervised and which site it belongs to, plus the version of the *service* that is running. A `start service` button appears here when the service is not. |
...
`edit` beside a scheduled process opens the schedule editor. Times follow the site's timezone, or this machine's local time when it is not paired to one:
