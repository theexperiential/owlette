...
<Callout type="info" title={"Cortex tool reference"}>

See the [Cortex Tools Reference](/docs/reference/cortex-tools) for the canonical tool list with parameters, tiers, and allowed commands.
...
Poll `GET /api/sites/{siteId}/machines/{machineId}/commands/{commandId}` for the command result. The status route returns `pending`, `in_progress`, `completed`, `failed`, or a 404/timeout. Cortex tool execution polls completion internally about every 1.5 seconds.
