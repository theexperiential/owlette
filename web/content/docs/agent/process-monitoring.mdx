...
Each process has a configurable `relaunch_attempts` limit. The desktop app creates new processes with 5 attempts, the dashboard with 3, and the service falls back to `MAX_RELAUNCH_ATTEMPTS` (3) when the stored value is missing or `0`. When the limit is reached:
