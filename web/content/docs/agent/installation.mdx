...
In the dashboard deployment flow, provide the installer URL, silent flags, optional verify path from a preset/template when available, and target machines. For remote installs, ensure the command includes `sha256_checksum`; the current deployment dialog does not collect it directly.
...
flowchart TD
    ...
    C --> D["agent auto-opens the pairing page and starts<br/>polling immediately — no prompt<br/>(browser skipped with --no-browser)"]
    D --> E["authorize from any device: the opened<br/>page, a phone, or the authorization URL"]
...
| method | when to use |
|--------|-------------|
| Auto-opened browser | Single-machine install when the target machine has a usable browser — the pairing page opens automatically |
