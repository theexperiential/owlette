...
| **Heartbeat + metrics** | Adaptive: ~5s while the desktop app's window is open, 30s while processes are active, 120s idle | Marks the machine online and reports CPU, memory, disk, and GPU usage |
...
6. Launch the desktop app (tray icon) as a user process
7. Launch hoot process as user process (if enabled)
8. Enter main loop
