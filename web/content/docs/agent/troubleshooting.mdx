...
**Opt out of temperature monitoring**: set `"temperature": { "enabled": false }` in `C:\ProgramData\Owlette\config\config.json` and restart the service. All sensor reads stop; temperature fields simply disappear from the dashboard. A missing `temperature` section means enabled.
