...
| **Service stdout** | `service_stdout.log` | NSSM-captured stdout from the service runner |
| **Service stderr** | `service_stderr.log` | NSSM-captured stderr — interpreter-level tracebacks that never reach the logger land here |
| **Cortex** | `cortex.log` | Cortex runtime and CLI fetch. Only written when Cortex is enabled. |
...
Main shared-logger files use rotating file handlers: 10 MB per file, 5 backups. NSSM rotates the two stdout/stderr files itself, daily or at 10 MB. `tmp/status_writer.log` rotates at 100 KB with 2 backups, and `pairing_debug.log` uses plain writes. Old main logs are auto-deleted after 90 days by default.
...
**Cause**: The WinRing0 driver that LibreHardwareMonitor uses for CPU/GPU temperatures is extracted at runtime to `C:\ProgramData\Owlette\python\python.sys` (and `pythonw.sys` whenever a windowless python host reads temperatures — Cortex and the user-session helpers still run under `pythonw.exe`) and loaded as kernel service `R0python` / `R0pythonw`. Defender's antivirus flags it as `VulnerableDriver:WinNT/Winring0` and may quarantine that `.sys` file. Process exclusions do not cover a kernel-driver file load, so the driver path itself must be excluded.
