...
Error responses are not cached. Streaming SSE/x-ndjson/chunked responses are not cached. Current Cortex `text/plain` streams are not excluded unless the helper is updated to detect them.
...
- Cortex conversation create and send require `Idempotency-Key`; rename and delete accept it optionally.
