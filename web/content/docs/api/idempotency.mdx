---
title: "idempotency"
description: "Mutating public API routes that opt into idempotency use an Idempotency-Key header so clients can retry safely after a timeout, 429, or transient 5xx without accidentally creating duplicate work."
---

**Last updated**: 2026-05-01

<Callout type="warning" title={"Route-specific support"}>

The `Idempotency-Key` header is honored only on routes that explicitly say so in the endpoint reference. It is ignored on `POST /api/roosts`, `POST /api/keys`, `POST /api/keys/{id}/rotate`, `PATCH /api/keys/{id}`, `DELETE /api/keys/{id}`, and `POST /api/chunks/upload-urls`.

</Callout>
Treat the route implementation as authoritative until OpenAPI is corrected; `POST /api/chunks/upload-urls` currently ignores `Idempotency-Key`. Sending the header to an unsupported route does not make that route replay-safe.
...
Error responses are not cached. Streaming SSE/x-ndjson/chunked responses are not cached. Current hoot `text/plain` streams are not excluded unless the helper is updated to detect them.
...
- hoot conversation create and send require `Idempotency-Key`; rename and delete accept it optionally.
...
All three errors use the standard `application/problem+json` envelope documented in [errors.md](/docs/api/errors).
...
- [errors.md](/docs/api/errors) for `idempotency_*` error bodies.
- [rate-limits.md](/docs/api/rate-limits) for retry timing and `Retry-After`.
- [pagination.md](/docs/api/pagination) for collection traversal conventions.
- The interactive reference at [`/docs/api`](/docs/api) for operation-level required headers.
