...
# edit scopes in place — the secret is untouched, and `scopes` replaces
# the key's list outright rather than merging into it
await client.keys.update(
    created["id"],
    scopes=[ApiKeyScope(resource="roost", id="*", permissions=["read", "write", "deploy"])],
)
...
async with Roost(token="owk_live_...", transport=httpx.MockTransport(handler)) as client:
