---
title: "errors"
description: "Public API errors use application/problem+json. Clients should branch on the stable code field, not on human-readable detail text."
---
...
```json
{
  ...
  "requestId": "5d7f7c5b-2ed6-4a56-9b2c-47dd7c84f957"
}
```
...
| `requestId` | Correlation id on canonical public API helper responses. Also emitted as `X-Request-Id` by those responses. |
...
| `tier_insufficient` | 403 | The site is on the core tier and the operation is pro-only. | Upgrade the site to pro. |
...
| `trial_expired` | 402 | The account's free trial ended, or its subscription was canceled. API, CLI, and SDK access is paused. | Choose a plan to restore access. |
...
The operation is available only on the pro tier, and the target site is on core. The body carries a `required` block naming the site and the tier it needs. Accounts on a free trial run at the pro feature level and never see this code.
...
The account that owns the site has no active subscription: its free trial ended, or a previous subscription was canceled. API, CLI, and SDK access is paused until a plan is chosen — no data is deleted, agents keep reporting, and the dashboard stays readable. The body carries `billingState` (`expired` or `canceled`) so clients can word their own prompt.

You get advance notice — see [the trial warning header](#trial-warning-header) below.

---

## trial warning header

While the owning account is still on its free trial, successful API-key responses carry an advisory header:

```http
X-Owlette-Billing-Warning: trial ends 2026-08-15T00:00:00.000Z; choose a plan to keep API access
```

The value is `trial ends <ISO-8601 UTC instant>; choose a plan to keep API access`. It is informational only — nothing is blocked while it is present. Once the trial ends, requests answer [`402 trial_expired`](#trial_expired) instead.

**When it appears**

| condition | header |
|---|---|
| API-key request, account trialing with a trial end date | present |
| account on an active (or past-due) subscription | absent |
| account trialing but the trial clock has not started yet | absent |
| trial ended or subscription canceled | absent — the request answers `402 trial_expired` |
| session / Firebase ID-token request (dashboard) | absent — the dashboard shows a trial banner instead |

**Where it appears.** On responses to the site-, machine-, roost-, and chat-scoped endpoints, once the request has cleared authentication, site access, and scope. It rides the same path as `X-Roost-Deprecation` and `X-Roost-Version-Missing`.

It is deliberately **not** attached to `401`, `403`, `404`, or `402` responses produced by those checks: a caller who cannot reach a site must learn nothing about that site's billing state, and the `402` already carries the remedy in its body. Treat the header as an advisory you may see on any successful response, not as a guaranteed field on every response — poll a successful endpoint if you need the trial deadline on demand.

**Client support**

- **CLI** (`@owlette/cli`) prints it to stderr once per command invocation. `--json` output on stdout is unaffected.
- **Node SDK** (`@owlette/sdk`) surfaces it through the optional `onBillingWarning` client option. Nothing is printed by default.
- **Python SDK** (`owlette-sdk`) surfaces it through the optional `on_billing_warning` client argument. Nothing is printed by default.
...
- quota failures (`402 quota_exceeded`) unless usage or plan state changed
- billing lockouts (`402 trial_expired`) until a plan is chosen
- tier failures (`403 tier_insufficient`) until the site is upgraded
- precondition failures (`409`, `412`) until you have refreshed state

For mutating retries, reuse the same `Idempotency-Key` only when the request method, path, query, and body are identical. See [idempotency.md](/docs/api/idempotency).

---

## request ids

Canonical public API helper responses include `requestId` in the body and `X-Request-Id` in the response headers. Some machine/process compatibility routes currently omit them until migrated. Log both the status/code and request id when present.
...
- [authentication.md](/docs/api/authentication) for auth and scope errors.
- [pagination.md](/docs/api/pagination) for pagination validation.
- [idempotency.md](/docs/api/idempotency) for replay and mismatch behavior.
- [rate-limits.md](/docs/api/rate-limits) for `Retry-After` and rate-limit headers.
