...
| `quota_exceeded` | 402 | Site storage quota would be exceeded. | Free usage or delete old roost versions. |
...
| `cortex_unavailable` | 400/423/503 | hoot could not stream because the target was missing, disabled, or offline. | Fix the target/enablement issue or retry after the machine reconnects. |
...
The request body included fields that are not accepted by that operation, such as extra fields on public hoot send or rename requests.
...
The request would exceed the site's roost storage quota.
...
hoot could not stream for the requested conversation because the target was invalid, disabled, or offline.
...
- quota failures (`402 quota_exceeded`) unless usage changed
