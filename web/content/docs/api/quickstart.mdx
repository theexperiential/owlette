---
title: "Owlette public API quickstart"
description: "This developer-preview quickstart creates a scoped API key, verifies it, lists your site inventory, queues a safe screenshot command on one machine, and polls the command result. It uses only the live REST API and common shell tools; no SDK publishing or external launch packaging is required."
---
...
Open the API keys settings page, click **create key**, and create a key with:
...
- Browse the interactive reference at [`/docs/api`](/docs/api).
- Read [`authentication.md`](/docs/api/authentication), [`pagination.md`](/docs/api/pagination),
  [`idempotency.md`](/docs/api/idempotency), [`errors.md`](/docs/api/errors), and
  [`rate-limits.md`](/docs/api/rate-limits) before wiring production automation.
- Add webhooks with [`webhooks.md`](/docs/api/webhooks) once you want event callbacks.
- Move from curl to the Node or Python SDK examples in
  [`examples/sdk-workflows.md`](/docs/api/examples/sdk-workflows) after the REST smoke
  workflow is green.
