...
- `OWLETTE_TOKEN` — api key with `roost=<roost-id>:rollback` for rollback calls. For probe testing, also grant `site=<site-id>:write` or use a separate probe token. To cover multiple roosts, grant per-roost rollback scopes or `roost=*:rollback`; `site=<site-id>:write` is not a rollback substitute.
