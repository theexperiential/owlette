---
title: "pagination"
description: "Public collection endpoints use cursor pagination. Cursors are opaque strings returned by the server; clients must round-trip them exactly and must not parse, sort, or construct them."
---
...
Routes using the shared pagination helper return `400 validation_failed` for invalid `page_size`; `/api/cortex/conversations` and `/api/chat` currently clamp or default invalid values, so clients should consult the endpoint reference for validation behavior.
...
The rendered endpoint reference at [`/docs/api`](/docs/api) is the source of truth for each operation's request parameters and response envelope.
...
See [errors.md](/docs/api/errors) for the full envelope contract.
