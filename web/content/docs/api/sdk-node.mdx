...
the official typescript sdk for the [Owlette public API](/docs/api/overview). wraps the rest surface with a typed resource tree, auto-retry, automatic `Idempotency-Key`, chunk-aware roost `push()`, stripe-style webhook verification, version-ref resolution, and progress events. if you can use `fetch` directly you can use this — it just adds the tedious bits.
