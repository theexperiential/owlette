...
**Created**: 2026-06-12 | **Status**: Canonical (interim home)
...
**INTERNAL — do not move into `docs/`** (that tree publishes to owlette.app/docs via Fumadocs). This doc discusses autologon, disabled screen lock, and elevated runners. When the e2e harness directory is created in Wave 1, promote this file to that directory's README and leave a pointer here.
...
```powershell
Unblock-File C:\path\to\Owlette-Installer-v*.exe
```
...
- [ ] **`pywin32` MUST stay at 306.** 310+ fails with `DLL load failed` against the embedded Py3.9/Win11 combo. This pin is enforced in `dev/video-tutorials/capture-native/requirements.txt` — if you hit DLL errors, check this first, do not "upgrade to fix".
...
# interactive console session present? (0xFFFFFFFF = no session -> GUI automation impossible)
python -c "import ctypes; sid = ctypes.windll.kernel32.WTSGetActiveConsoleSessionId(); print('console session:', sid); assert sid != 0xFFFFFFFF"
```
...
- [ ] OBS (recommended capture) or ffmpeg on PATH for the `ScreenRecorder` (gdigrab) fallback.
- [ ] Scene workflow, `DUMP=1` control-tree tuning, and recorder helpers: see `dev/video-tutorials/capture-native/README.md`.
- [ ] Disposable machine only — installs/uninstalls Owlette repeatedly.
...
- [ ] **Second snapshot for the upgrade leg**: version N-1 installed + paired + heartbeating. Document the rotation procedure when re-baselining (and exclude its baked-in refresh token from cloud teardown sweeps — see `plan.md`).

---

## Maintenance

This doc is owned by the full-machine-e2e initiative while it lives here; after Wave 1 promotion, the harness directory owns it. If the capture-native rig needs something machine-level that isn't here, **add it here** — never to the capture-native README.
