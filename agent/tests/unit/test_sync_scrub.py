...
from sync_manifest import Manifest, ManifestChunk, ManifestFile, MANIFEST_MEDIA_TYPE
...
def _mk_manifest_file(path: str, chunk_data_list):
    ...
    for d in chunk_data_list:
        chunks.append(ManifestChunk(hash=_hash(d), size=len(d)))
        total += len(d)
...
def _setup_committed_distribution(tmp_path, files_data):
    """
    set up: a committed distribution + on-disk files matching the manifest +
    a SyncState with the right rows.

    files_data: dict[path -> [chunk_bytes, ...]]
    returns: (state, dist_id, extract_root, manifest)
    """
    ...
    for path, chunk_data_list in files_data.items():
        f = _mk_manifest_file(path, chunk_data_list)
        files.append(f)
    ...
    dist_id = state.start_distribution(
        site_id='s', folder_id='f', manifest_id='m', manifest_url='https://r2/m.json',
    ...
    return state, dist_id, str(extract), manifest
...
def test_healthy_distribution_returns_no_drift(tmp_path):
    state, dist_id, extract, manifest = _setup_committed_distribution(
        tmp_path, {'a.toe': [b'hello world']}
    )
    try:
        with patch('sync_scrub.fetch_manifest', return_value=manifest):
