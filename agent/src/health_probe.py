...
# Check 3 and wait_for_network() share one connect impl so both agree on what
# "reachable" means.
NETWORK_PROBE_PORT = 443
NETWORK_PROBE_TIMEOUT = 5.0

# wait_for_network() cadence: quick early gaps (a nearly-up NIC comes up in
# seconds), then flat 5s so a 90s budget costs a bounded number of probes.
...
def extract_host(api_base: str) -> Optional[str]:
    ...
    try:
        # Hand-rolled to stay stdlib-light.
...
class HealthProbe:
    ...
    def _check_config(self):
        ...
        if not firebase.get('enabled') or not firebase.get('site_id'):
            # Disabled or site-less isn't an error here; the token check
            # catches auth_error.
            return True, True, ""
    ...
    def _check_network(self):
        ...
        if not host:
            # Unparseable host — skip rather than false-fail.
            return True, ""
        ...
        if not isinstance(error, OSError):
            # Never block startup on a network probe failure.
            return True, ""
