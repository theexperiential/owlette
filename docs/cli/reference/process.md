...
- `0` — success
- `1` — generic error (network, api 5xx, 409 `duplicate_process_name`, 404 `process_not_found`, 403 `scope_insufficient`, agent-side failure on a queued command)
- `2` — usage error (missing `--site` / `--machine`, no field flags on `update`, bad `--mode` or `--launch-mode` value, malformed `--blocks` json, refusal to delete on a non-tty without `--yes`, no token configured)
