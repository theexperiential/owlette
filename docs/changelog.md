...
- **Persistent WMI worker for disk IO** — replaced the per-call `ThreadPoolExecutor` + fresh `wmi.WMI()` instantiation with a long-lived worker thread that holds the WMI connection across calls. Eliminates the ~350ms per-tick instantiation cost AND the COM apartment leak (the previous pattern called `pythoncom.CoInitialize()` in STA mode without a paired `CoUninitialize()` on the disposable worker thread). Bumped the default timeout 2s → 5s so the metrics loop captures (rather than skips) the brief perflib-provider lock contention that occurs when the BITS service flips state during Windows Update polling.
