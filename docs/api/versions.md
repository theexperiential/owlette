...
A version is an immutable JSON body that lists the files in a roost publish and
the chunk hashes needed to reconstruct each file. The public routes use that
body to publish history, resolve version refs, list files, compute diffs, roll
back the current pointer, and trigger deployments.
...
| `scheduleAt` | no | ISO-8601 timestamp. Future scheduling is stored but not auto-fired yet. |
...
- [chunks.md](./chunks.md) - check, upload, and download content-addressed chunks.
- [errors.md](./errors.md) - shared problem envelope and recovery guidance.
- [sdk-node.md](./sdk-node.md) and [sdk-python.md](./sdk-python.md) - high-level `roosts.push()` workflows.
