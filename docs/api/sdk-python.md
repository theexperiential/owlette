...
**Package**: [`owlette-roost`](https://pypi.org/project/owlette-roost/) · python ≥ 3.10 · single runtime dep on `httpx`

the official async python sdk for the [roost api](./overview.md). `async`-first (built on `httpx.AsyncClient`), typed with dataclasses + `py.typed` marker, and behaviour-compatible with [`@owlette/roost`](./sdk-node.md) — the resource tree, method names, progress events, error codes, and signature scheme are identical across the two. if the node docs say `roost.roosts.push(...)`, the python sdk calls it `await client.roosts.push(...)`.
...
# or: poetry add owlette-roost
# or: uv pip install owlette-roost
```
