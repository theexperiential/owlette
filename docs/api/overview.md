...
**2. manifests — immutable snapshots.**
a manifest is an oci-shaped document listing every file in a deployment and the chunk digests that make it up. once published, a manifest is never mutated; a new publish produces a new manifest with its own digest. manifests are what you actually compare, diff, and roll back between.

**3. roosts — named, versioned bundles.**
a roost is the deployment unit: a stable id (like `roost_lobby_td`) that points at a *current* manifest and remembers its history. publishing is a pointer move — the bytes are already in r2 as chunks; the manifest references them; the roost's pointer flips atomically from the old manifest to the new one. rollback is the same flip in reverse. roosts have a target list (which machines should run them) and belong to a site.

**4. deployments — targeted fan-out to machines.**
when a roost's current manifest changes, agents on target machines diff what they have against the new manifest, pull only the missing chunks, verify each hash, and swap the on-disk tree in one move. a deployment is the record of this rollout: machines, roles (canary vs fleet), per-machine state, bytes transferred. you can also trigger one explicitly with scheduling, dry-run, and canary strategy.

the flow, end to end: chunk your files → upload the missing chunks → publish a manifest pointing at them → the roost's pointer flips → agents fan out and apply → rollback is one api call away.
...
- you're exploring: "which manifest is currently on machine-a7f3, and when did it last sync?"
...
from there, deep-dive into the [chunks](./chunks.md), [manifests](./manifests.md), and [webhooks](./webhooks.md) guides as you need them. the full endpoint reference lives in [openapi.yaml](../../web/openapi.yaml) and is machine-readable.
